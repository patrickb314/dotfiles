...
- Generally prefer amending existing commits but only if they haven't already been pushed. This is especially true for functionality fixes---update the commit message to note the fix instead of creating follow-up fix commits if the fix hasn't already been pushed. If it has already been pushed, make a follow up commit.
